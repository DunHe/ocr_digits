...
def cropDigits(img, x, y, h, w, num_chars):
    ...
    cropImg = np.zeros((h, w, 3), np.uint8)
    ...
    for i in range(num_chars):
        _digit = np.zeros((h, min_char_width, 3), np.uint8)
        _digit = cv2.cvtColor(_digit, cv2.COLOR_BGR2GRAY)
